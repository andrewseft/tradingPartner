...
class EmailManager
{
     ...
     * @param $url
     */
    public function sendEmail($id,$user,$url,$notification){
        $when = now()->addSeconds(Constant::SHOULD_QUEUE);
        $user->notify((new EmailNotification($id,$user,$url,$notification))->delay($when));
    }
     ...
     * @param $url
     */
    public function sendOtp($id,$user,$url){
         
        $when = now()->addSeconds(Constant::SHOULD_QUEUE);
        $user->notify((new OTPNotification($id,$user,$url,null))->delay($when));
    }
