...
@if($data->count() || $request->query('created_at_from') || $request->query('created_at_to')|| $request->query('keyword') || $request->query('status'))
    <div class="row no-gutters">
        <div class="col-lg-12 col-md-12 col-sm-12">
            <div class="card">
                <div class="body">
                {{ Form::model($faq, ['url' => route('admin.faq'),'class' => 'search_form','method'=>'get']) }}
                    <div class="row table_search_form align-items-center">
                        <div class="col input_feild py-1">
                            {{ Form::text('keyword',null , array_merge(['class' =>'form-control','placeholder'=>__('Search by questions')])) }}
                        </div>
                        ...
                        <div class="col date_feild py-1">
                            ...
                            <a href="javascript:void(0)" class="{{isset($faq['created_at_from']) ? 'date_feild_close_show': 'date_feild_close'}}"><i class="fas fa-times-circle"></i></a>
                        </div>
                        <div class="col date_feild py-1">
                            {{ Form::text('created_at_to',null , array_merge(['class' =>'form-control date','placeholder'=>__('To'),'readonly'=>true])) }}
                            <a href="javascript:void(0)" class="{{isset($faq['created_at_to']) ? 'date_feild_close_show': 'date_feild_close'}}"><i class="fas fa-times-circle"></i></a>
                        </div>
                        ...
                        <div class="col py-1">{{ link_to( route('admin.faq'),'<i class="fas fa-redo-alt"></i>',['class'=>'btn btn-danger'],null, false)}}</div>
                        <div class="col py-1">{{ link_to( route('admin.faq.add'),'<i class="fas fa-plus"></i>',['class'=>'btn btn-success'],null, false)}}</div>
                        <div class="col py-1">{{ link_to( route('admin.faq.sortable'),'<i class="fas fa-align-justify"></i>',['class'=>'btn btn-secondary','data-toggle'=>'tooltip','data-original-title'=>__('Sortable')],null, false)}}</div>
                    </div>
                {{ Form::close() }}
                </div>
            </div>
                ...
                @if($data->count() || $request->query('created_at_from') || $request->query('created_at_to')|| $request->query('keyword') || $request->query('status'))
                    @if($data->count())
                        <table class="table table-hover m-b-0">
                            <thead>
                                <tr>
                                    <th>@sortablelink('title',__('Questions '))</th>
                                    <th>{{__('Answer')}}</th>
                ...
                @else
                    <center class=" mb-4 mt-2">
                        ...
                        <div class="col-sm-6 col-md-4 col-lg-2">
                            {{ link_to( route('admin.faq.add'),__('Create New'),['class'=>'btn btn-raised m-b-10 btn-primary btn-block waves-effect'],null, false)}}
                        </div>
                    </center>
                @endif
                </div>
...
<script>
    ...
    $(document).on("change",".status_click",function(t){t.preventDefault();var c=$(this).data("url"),e=$(this).prop("checked");console.log($(this).prop("checked")),$.ajax({url:c,data:{status:e},faq:"get",success:function(t){}})});
</script>
@stop
