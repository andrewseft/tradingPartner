...
@if($data->count() || $request->query('keyword') || $request->query('email')|| $request->query('investmentCapital') || $request->query('created_at_from') || $request->query('created_at_to') || $request->query('status') || $request->query('plan') || $request->query('is_kyc'))
    <div class="row no-gutters">
        <div class="col-lg-12 col-md-12 col-sm-12">
            <div class="card">
                <div class="body">
                {{ Form::model($user, ['url' => route('admin.statement'),'class' => 'search_form','method'=>'get']) }}
                    <div class="row table_search_form align-items-center">
                        ...
                        <div class="col py-1">{{ link_to( route('admin.statement'),'<i class="fas fa-redo-alt"></i>',['class'=>'btn btn-danger'],null, false)}}</div>
                        <div class="col py-1">{{ link_to( route('admin.redeem.excel',$user),'<i class="far fa-file-excel"></i>',['class'=>'btn btn-success'],null, false)}}</div>
                    </div>
                {{ Form::close() }}
                </div>
            </div>
