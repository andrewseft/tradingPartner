            ...
            <div class="col-lg-8 col-md-12">
                <div class="card">
                    <ul class="nav nav-tabs">
                        ...
                        <li class="nav-item"><a href="{{route('admin.redeem.userRedeem',['id' => Helper::encode($user->id)])}}" class="nav-link active">{{__('Redeem')}}</a></li>
                                    ...
                                    @if($data->count() || $request->query('keyword') || $request->query('created_at_from') || $request->query('created_at_to'))
                                        <div class="row no-gutters">
                                            <div class="col-lg-12 col-md-12 col-sm-12">
                                                <div class="card">
                                                    <div class="body">
                                                        {{ Form::model($redeemData, ['url' => route('admin.redeem.userRedeem',['id' => Helper::encode($user->id)]),'class' => 'search_form','method'=>'get']) }}
                                                            <div class="row table_search_form align-items-center">
                                                                <div class="col input_feild py-1">
                                                                    {{ Form::text('keyword',null , array_merge(['class' =>'form-control','placeholder'=>__('Search by plan')])) }}
                                                                </div>
                                                                ...
                                                                <div class="col py-1">{{ link_to( route('admin.redeem.userRedeem',['id' => Helper::encode($user->id)]),'<i class="fas fa-redo-alt"></i>',['class'=>'btn btn-danger'],null, false)}}</div>
                                                                <!-- <div class="col py-1">{{ link_to( route('admin.redeem.excel',$redeemData),'<i class="far fa-file-excel"></i>',['class'=>'btn btn-success'],null, false)}}</div> -->
                                                            </div>
                                                        {{ Form::close() }}
                                                    </div>
                                                </div>
                                                        ...
                                                        <table class="table table-hover m-b-0">
                                                            <thead>
                                                                <tr>
                                                                    ...
                                                                    <th>@sortablelink('realized',__('Realized P&L '))</th>
                                                                ...
                                                                @if($data->count())
                                                                    @foreach($data as $key => $value)
                                                                        <tr>
                                                                            ...
                                                                            <td> {{$value->qty}}</td>
                                                                            <td>{{number_format($value->realized,2)}}</td>
                                                                            <td>{{Helper::date($value->created_at)}}</td>
                                                                            <td>
                                                                            {{ link_to('#javascript:void(0)','<i class="fa fa-eye"></i>',['data-title'=>$value->user->fullName.'('.$value->plan->title.')','data-url'=>route('admin.redeem.view', ['id' => Helper::encode($value->id)]),'onclick'=>'return false;','class'=>'btn btn-default waves-effect  waves-float item_view','data-toggle'=>'tooltip','data-original-title'=>__('View')],null, false)}}&nbsp;
                                                                            </td>
                                                                        </tr>
                                                                    @endforeach
                                                                @endif
