    ...
    public function rules(Request $request)
    {
        ...
        return [
            'translation.*.title'    => ['required','max:150',function ($attribute, $value, $fail) use ($data){
                ...
                $detail = [];
                if(isset($data['id'])){
                    $detail = PageTranslation::where('id','!=',$data[$name[0]][$name[1]]['id'])->where('title',$value)->where('locale',$data[$name[0]][$name[1]]['locale'])->first();
                }else{
                    $detail = PageTranslation::where('title',$value)->where('locale',$data[$name[0]][$name[1]]['locale'])->first();
                }
