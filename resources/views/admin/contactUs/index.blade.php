...
@if($data->count() || $request->query('keyword') || $request->query('created_at_from') || $request->query('created_at_to'))
    <div class="row no-gutters">
        <div class="col-lg-12 col-md-12 col-sm-12">
            <div class="card">
                <div class="body">
                {{ Form::model($contactUs, ['url' => route('admin.contactUs'),'class' => 'search_form','method'=>'get']) }}
                    <div class="row table_search_form align-items-center">
                        <div class="col input_feild py-1">
                            {{ Form::text('keyword',null , array_merge(['class' =>'form-control','placeholder'=>__('Search by name, email, number')])) }}
                        </div>
                        ...
                        <div class="col date_feild py-1">
                            ...
                            <a href="javascript:void(0)" class="{{isset($contactUs['created_at_from']) ? 'date_feild_close_show': 'date_feild_close'}}"><i class="fas fa-times-circle"></i></a>
                        </div>
                        <div class="col date_feild py-1">
                            {{ Form::text('created_at_to',null , array_merge(['class' =>'form-control date','placeholder'=>__('To'),'readonly'=>true])) }}
                            <a href="javascript:void(0)" class="{{isset($contactUs['created_at_to']) ? 'date_feild_close_show': 'date_feild_close'}}"><i class="fas fa-times-circle"></i></a>
                        </div>
                        ...
                        <div class="col py-1">{{ link_to( route('admin.contactUs'),'<i class="fas fa-redo-alt"></i>',['class'=>'btn btn-danger'],null, false)}}</div>
                    </div>
                {{ Form::close() }}
                </div>
            </div>
            ...
            <div class="card product_item_list">
                ...
                @else
                    <center class=" mb-4 mt-2">
                        {!! Html::image('img/man.png','contactUs',['class'=>'img-circle','width'=>60])  !!}
