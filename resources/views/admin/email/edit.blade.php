        ...
        <div class="row">
            ...
            <div class="col-lg-5 col-md-6 col-sm-12">
                <ul class="breadcrumb float-md-right">
                    ...
                    <li class="breadcrumb-item"><a href="{{route('admin.email')}}">{{$title}}</a></li>
        ...
        <div class="card">
            <div class="body">
                <div class="tab nav-bt">
                    {{ Form::model($email, ['url' => route('admin.email.create'),'class' => 'user','method'=>'post']) }}
                    {{ Form::hidden('id')}}
                        <div class="tab-content">
                            @foreach(Helper::language() as $languagekey => $language)
                                <div class="tab-pane fade py-3 {{ $languagekey == App::getLocale() ? ' active' : '' }} show" id="home-{{$languagekey}}" role="tabpanel" aria-labelledby="home-{{$languagekey}}-tab">
                                    @include('admin.email.from')
                                </div>
                            @endforeach
                        </div>
                        {{ Form::button(__('Update'),['class'=>'btn btn-primary','id'=>'submitButton','type'=>'submit']) }}
                        <a href="{{route('admin.email')}}" class="btn btn-light ml-3">{{__('Back')}}</a>
                    {{ Form::close() }}
                </div>
            </div>
        </div>
