...
@if($data->count() || $request->query('keyword') || $request->query('status'))
    <div class="row no-gutters">
        <div class="col-lg-12 col-md-12 col-sm-12">
            <div class="card">
                <div class="body">
                    {{ Form::model($plan, ['url' => route('admin.plan'),'class' => 'search_form','method'=>'get']) }}
                        <div class="row table_search_form align-items-center">
                            ...
                            <div class="col py-1">{{ link_to( route('admin.plan'),'<i class="fas fa-redo-alt"></i>',['class'=>'btn btn-danger'],null, false)}}</div>
                            <div class="col py-1">{{ link_to( route('admin.plan.add'),'<i class="fas fa-plus"></i>',['class'=>'btn btn-success'],null, false)}}</div>
                            <div class="col py-1">{{ link_to( route('admin.plan.sortable'),'<i class="fas fa-align-justify"></i>',['class'=>'btn btn-secondary','data-toggle'=>'tooltip','data-original-title'=>__('Sortable')],null, false)}}</div>
                        </div>
                    {{ Form::close() }}
                </div>
            </div>
                        ...
                        <table class="table table-hover m-b-0">
                            <thead>
                                <tr>
                                    ...
                                    <th>@sortablelink('amount',__('Current Price'))</th>
                                    <th>@sortablelink('qty',__('Qty '))</th>
                                    <th>@sortablelink('pl',__('Today P&L '))</th>
                ...
                @else
                    <center class=" mb-4 mt-2">
                        ...
                        <div class="col-sm-6 col-md-4 col-lg-2">
                            {{ link_to( route('admin.plan.add'),__('Create New'),['class'=>'btn btn-raised m-b-10 btn-primary btn-block waves-effect'],null, false)}}
                        </div>
                    </center>
                @endif
                </div>
