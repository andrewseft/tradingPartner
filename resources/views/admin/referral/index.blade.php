        ...
        @if($data->count() || $request->query('keyword') || $request->query('created_at_from') || $request->query('created_at_to'))
            <div class="row no-gutters">
                <div class="col-lg-12 col-md-12 col-sm-12">
                    <div class="card">
                        <div class="body">
                            {{ Form::model($useData, ['url' => route('admin.referral'),'class' => 'search_form','method'=>'get']) }}
                                <div class="row table_search_form align-items-center">
                                    ...
                                    <div class="col py-1">{{ link_to( route('admin.referral'),'<i class="fas fa-redo-alt"></i>',['class'=>'btn btn-danger'],null, false)}}</div>
                                </div>
                            {{ Form::close() }}
                        </div>
                    </div>
                                    ...
                                    @if($data->count())
                                        @foreach($data as $key => $value)
                                            <tr>
                                                ...
                                                <td>{{ucfirst($value->from->first_name)}}</td>
                                                <td>{{$value->referral_commission}}%</td>
